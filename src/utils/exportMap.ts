  ...
  return new Promise((resolve, reject) => {
    ...
    map.once('render', () => {
      ...
      img.onload = () => {
        ...
        const legendW = 260;
        const legendH = legendPad * 2 + rows * legendLineH + 8;
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(legendX + legendPad, legendY + legendPad + 14);
        ctx.lineTo(legendX + legendW - legendPad, legendY + legendPad + 14);
        ctx.stroke();
        ctx.restore();
        ...
        legendItems.forEach((item, i) => {
          ...
          const row = i % rows;
          const centerY = legendY + legendPad + 22 + row * legendLineH + (legendLineH / 2);
          ...
          if (item.dash !== undefined) {
            ...
            ctx.lineWidth = item.dash ? 2 : 1.5;
            if (item.dash) ctx.setLineDash([5, 3]);
            ctx.beginPath();
            ctx.moveTo(ix, centerY);
            ctx.lineTo(ix + 24, centerY);
            ctx.stroke();
            ctx.setLineDash([]);
          } else if (item.img) {
            // Team icon
            ctx.drawImage(item.img, ix - 2, centerY - 7, 14, 14);
          } else {
            // Node symbol
            ctx.font = '12px sans-serif';
            ctx.fillStyle = item.color;
            ctx.fillText(item.node, ix + 4, centerY + 1);
          }

          ctx.font = '500 10px Inter, sans-serif';
          ctx.fillStyle = '#C8D0E0';
          ctx.fillText(item.label, ix + 28, centerY);
          ctx.restore();
        });
